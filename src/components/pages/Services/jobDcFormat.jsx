...
const JobDeliveryChallan = ({ dcNumber }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/jobdcentry/full/${encodeURIComponent(dcNumber)}`);
      ...
      >
        ...
        <div className="relative min-h-[68px] px-3 py-1">
          ...
          <div>
            ...
            <p className="text-[12px] mt-3 font-bold uppercase relative top-4">Receiver's Signature</p>
          </div>
...
export default JobDeliveryChallan;
