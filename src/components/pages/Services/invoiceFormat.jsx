  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (data && data.header) {
          setInvoice({ ...data.header, items: data.items || [] });
        }
      } catch (error) {
      ...
      >
        ...
        <div className="flex border-2 border-black mx-2 flex-shrink-0" style={{ height: "217px" }}>
          ...
          <div className="w-[55%] p-3 border-r-2 border-black flex flex-col justify-center">
            <InvoiceAddressBlock
              name={invoice?.customer_name || invoice?.client?.customer_name}
              address={invoice?.client_address || invoice?.client?.address}
              state={invoice?.client_state || invoice?.client?.state}
              pincode={invoice?.client_pincode || invoice?.client?.pincode}
              phone={invoice?.client_phone || invoice?.client?.phone}
              gst={invoice?.client_gst || invoice?.client?.gst_number}
          ...
          <table className="w-full border-collapse flex-1" style={{ display: "flex", flexDirection: "column" }}>
            <thead style={{ display: "table", width: "100%", tableLayout: "fixed" }}>
              <tr className="border-b-2 border-black bg-gray-50">
                ...
                <th className="border-r-2 border-black p-2 w-[48%] text-left text-[12px] font-extrabold uppercase">Description</th>
            ...
            <tbody style={{ display: "table", width: "100%", tableLayout: "fixed", flex: 1 }}>
              {items.map((item, index) => (
                <tr key={index} className="min-h-[30px]">
                  ...
                  <td className="border-r-2 border-black px-3 text-[11px] font-bold uppercase w-[48%]">{item.item_name}{item.serial_no ? ` (${item.serial_no})` : (item.sl_no ? ` (${item.sl_no})` : "")}</td>
                  <td className="border-r-2 border-black text-center text-[11px] font-bold w-[12%]">{item.hsn_number || item.hsn_code || "998719"}</td>
        ...
        <div className="mx-2 border-2 border-black border-t-0 flex flex-shrink-0 h-[210px]">
          ...
          <div className="w-[45%] flex flex-col leading-5 relative justify-between">
            <div className="absolute top-0 bottom-0 left-[245px] border-l-2 border-black"></div>
            <div className="flex-1 flex flex-col justify-start">
              ...
              {discount > 0 && (
                <div className="flex px-3 py-0.5">
                  ...
                  <span className="w-[110px] text-right text-[14px] font-bold text-red-600">-{discount.toFixed(2)}</span>
                </div>
              )}
              <div className="flex px-3 py-0.5">
                <span className="flex-1 text-right text-[13px] font-bold pr-4">TAXABLE</span>
                <span className="w-[110px] text-right text-[14px] font-bold">{taxable.toFixed(2)}</span>
              </div>
              {(() => {
