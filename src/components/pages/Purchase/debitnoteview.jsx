  ...
  return (
    ...
    <div className="min-h-screen">
      <div className="flex justify-center  min-h-screen ">
        ...
        <div className="w-[800px] h-[1400px] bg-white relative shadow-lg overflow-hidden">
          ...
          <div className="relative z-10 px-10 ">
            <div className="flex justify-between ">
              <div>
                ...
                <h4 className="font-medium text-black">{debitnote?.client?.customer_name}</h4>
                <p className="text-black">{debitnote?.client?.address},{debitnote?.client?.state}-{" "}{debitnote?.client?.pincode}<br />
                </p>

                <div className="mt-5">
                  <p className="text-black"><span className="text-medium">Phone :</span>{debitnote?.client?.phone}</p>
                  <p className="text-black"><span className="text-medium"> Email : {debitnote?.client?.email}</span></p>
                  <p className="text-black"><span className="text-medium"> GSTN : {debitnote?.client?.gst_number}</span></p>

                </div>
              </div>
              {/* debitnote Order Details */}
              <div className="border-befor-left">
                <h2 className="font-bold border-b pb-2 text-center border-gray-800">DEBITNOTE</h2>
                <h4 className="font-medium mt-6">Dn No : <span className="text-black text-sm">
                  {debitnote?.dn_number}</span></h4>
                <h4 className="font-medium">Dn Date : <span className="text-black text-sm">{debitnote?.dn_date}</span></h4>
                 <h4 className="font-medium">Bill No : <span className="text-black text-sm">{debitnote?.bill_no}</span></h4>
                <h4 className="font-medium">Bill Date : <span className="text-black text-sm">{debitnote?.bill_date}</span></h4>
            ...
            <div className="mt-5 w-full">
              ...
              <div className="w-full p-2 border-t-0 flex justify-end border">
                <div className="w-[300px] text-sm ">
                  ...
                  <div className="flex justify-between px-4 py-2 ">
                    ...
                    <span className="font-medium">{debitnote.subtotal}</span>
                  </div>

                  <div className="flex justify-between px-4 py-2 ">
                    <span>SGST @18.00%</span>
                    <span>{debitnote.sgst}</span>
                  </div>
                    <div className="flex justify-between px-4 py-2 ">
                    <span>CGST @18.00%</span>
                    <span>{debitnote.cgst}</span>
                  </div>
                    <div className="flex justify-between px-4 py-2 ">
                    <span>IGST @18.00%</span>
                    <span>{debitnote.igst}</span>
                  </div>

                  <div className="flex justify-between px-4 py-2 font-bold">
                    <span>NET TOTAL</span>
                    <span>{debitnote.grandTotal}</span>
                  </div>
              ...
              <div className="border p-2 border-t-0">
                <h2 className="font-bold text-black "><span className="text-sm text-gray-600"> Rupees </span> : {amountInwords(debitnote.grandTotal || 0).toUpperCase()}</h2>
              </div>
