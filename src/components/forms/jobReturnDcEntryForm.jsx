...
const Api_url = `${API_BASE_URL}/jobreturndc`;
const REMARKS_OPTIONS = ["For Service", " For Re Service", "For Testing"];
...
const JobReturnDcEntryForm = () => {
    ...
    const loadReturnForEdit = async (returnDc) => {
        try {
            ...
            if (data.job_dc_no) {
                const sourceRes = await fetch(`${API_BASE_URL}/jobdcentry/editdc/${encodeURIComponent(data.job_dc_no)}`);
                if (sourceRes.ok) {
                    const sourceData = await sourceRes.json();
                    setSelectedJobItems(sourceData.items || []);
                    settabledata(prev => prev.map(loadedItem => {
                        const sourceItem = (sourceData.items || []).find(si => si.id === loadedItem.job_dc_item_id);
                        return {
                            ...loadedItem,
    ...
    const deleteReturnDc = async () => {
        ...
        if (!window.confirm(`Delete Return DC ${loadReturnNo}? This will revert the Job DC item status and pending quantities.`)) return;
        ...
        <><div className="min-h-screen bg-gray-50 p-6 font-sans">
            ...
            <div className="max-w-[1500px] mx-auto bg-white p-8 shadow-sm border border-gray-200 rounded-xl">
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-xl font-black text-black tracking-tight">JOB RETURN ENTRY</h2>
                        <p className="text-[12px] text-gray-400 mt-1">Select Pending Job DC → Deduct Quantity → Save Return.</p>
                    </div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 mb-5">
                            ...
                            <div className="grid grid-cols-5 gap-5">
                                ...
                                <div className="relative" ref={jobRef}>
                                    ...
                                    >
                                        <span>{formData.job_dc_no || "Select DC No..."}</span>
                                        <svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </div>
                                            ...
                                            {pendingJobs
                                                ...
                                                .map((job) => (
                                                    <div
                                                        ...
                                                        className="px-4 py-3 hover:bg-blue-50 cursor-pointer border-b border-gray-50 last:border-0"
                                                    >
                                                        <div className="text-[13px] font-bold text-gray-900">{job.job_dc_no}</div>
                                                    </div>
                                                ))}
                                        </div>
                                    )}
                                    {jobOpen && pendingJobs.filter(j => j.customer_name === formData.customer_name).length === 0 && (
                                        <div className={`${dropdownCls} p-4 text-center text-[12px] text-gray-400 font-semibold`}>
                                            No pending jobs for this customer.
                                        </div>
                                    )}
                                </div>
                            ...
                            {tabledata.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="py-14 text-center">
                                        ...
                                        <p className="text-[12px] text-gray-300 mt-1">Select a Job DC and items to begin.</p>
                                    </td>
                                </tr>
                            ) : (
                ...
                {/* Stored Job Return DC Entries Registry */}
                <div className="mt-10">
                    <div className="flex items-center gap-3 mb-4">
                        <h2 className="text-base font-black text-gray-800">
                            {formData.customer_name
                                ? `Job Return DC Entries — ${formData.customer_name}`
                                : "All Job Return DC Entries"}
                        </h2>
                        ...
                        <table className="w-full border-collapse">
                            ...
                            <tbody>
                                {allReturnDc.length === 0 ? (
                                    <tr><td colSpan={7} className="p-8 text-center text-gray-400 text-sm">No Job Return DC entries found.</td></tr>
                                ) : (
                                    allReturnDc.map((dc, i) => (
                                        <tr key={i} className="border-b border-gray-100 hover:bg-gray-50 font-normal">
                                            ...
                                            <td className="p-3 text-[12px] border-r font-semibold">{dc.job_dc_no}</td>
...
export default JobReturnDcEntryForm;
