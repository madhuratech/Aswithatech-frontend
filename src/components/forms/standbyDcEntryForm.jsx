...
const StandbyDcEntryForm = () => {
    ...
    const fetchNextDcNo = async () => {
        try {
            ...
            setFormData(p => ({ ...p, standby_dc_no: data.dc_no }));
        } catch {
            console.error("Could not fetch Standby DC No");
        }
    };
    ...
    const SaveDc = async () => {
        ...
        try {
            ...
            setSavedDcNo(formData.standby_dc_no);
            setShowSuccessModal(true);
            fetchAllDc();
        } catch (error) {
            toast.error(error.message || "Failed to Save Standby DC");
        }
    };
    ...
    const loadDcForEdit = async (dcNumber) => {
        try {
            ...
            setLoadDcId(h.id);
            toast.success("Standby DC Loaded");
            setEditOpen(false);
        } catch {
            toast.error("Failed to Load Standby DC");
        }
    };

    const deleteDc = async () => {
        if (!loadDcId) { toast.error("Load a Standby DC first to delete"); return; }
        if (!window.confirm("Delete this Standby DC? This cannot be undone.")) return;
        try {
            const res = await fetch(`${Api_url}/deletedc/${encodeURIComponent(formData.standby_dc_no)}`, { method: "DELETE" });
            if (!res.ok) throw new Error("Failed");
            toast.success("Standby DC Deleted");
            resetAll();
            fetchAllDc();
        } catch {
            toast.error("Failed to Delete Standby DC");
        }
    };

    const handlePrintClick = () => {
        if (loadDcId || savedDcNo) {
            setViewDcNo(formData.standby_dc_no);
            setDcModalMinimized(false);
            setShowDcFormat(true);
        } else {
    ...
    const handleEditClick = () => {
        ...
        toast.info("Please search or select a Standby DC from the registry to edit.");
    };
    ...
    return (
        <><div className="min-h-screen bg-gray-50 p-6 font-sans">
            ...
            {showSuccessModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-8 shadow-2xl w-full max-w-sm text-center">
                        ...
                        <h2 className="text-xl font-black text-gray-800 mb-1">Standby DC Saved Successfully!</h2>
                        <p className="text-sm text-gray-500 mb-1">Standby DC has been created.</p>
            ...
            >
                <StandbyDeliveryChallan key={viewDcNo} dcNumber={viewDcNo} />
            </ServiceWindowModal>
            ...
            <div className="max-w-[1500px] mx-auto bg-white p-8 shadow-sm border border-gray-200 rounded-xl">
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-xl font-black text-black tracking-tight">STANDBY DC ENTRY</h2>
                        <p className="text-[12px] text-gray-400 mt-1">Customer → Standby DC → Items → Save</p>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-5">
                        ...
                        <div>
                            <label className={labelCls}>
                                Standby DC No <span className="ml-1 text-[10px] text-blue-500 font-black normal-case">Auto</span>
                            </label>
                            <input type="text" value={formData.standby_dc_no} readOnly className={roInputCls} />
                        </div>
                    ...
                    <table className="w-full border-collapse">
                        <thead className="sticky top-0 z-10 bg-gray-50">
                            <tr className="bg-gray-50 border-b border-gray-200">
                                {["#", "Item Name", "Serial Number", "Qty", "Hsn", "UOM", "Remarks", "Actions"].map((h, i) => (
                            ...
                            {tabledata.length === 0 ? (
                                ...
                                tabledata.map((item, idx) => (
                                    <tr key={idx} className={`border-b border-gray-100 ${editIndex === idx ? "bg-blue-50" : "hover:bg-gray-50"}`}>
                                        ...
                                        <td className="p-3 text-[12px] border-r w-[10%]">{item.hsn}</td>
                                        <td className="p-3 text-[12px] border-r w-[10%]">{item.uom}</td>
                                        <td className="p-3 text-[12px] border-r w-[10%]">{item.remarks || "—"}</td>
                        ...
                        <tfoot className="sticky bottom-0 z-10">
                            <tr>
                                <td colSpan={8} className="px-4 py-3">
                                    <div className="flex items-center ml-[35%] gap-2">
                ...
                {/* Operations Footer (Placeholder Purpose field as requested) */}
                <div className="mt-6 bg-gray-50 rounded-xl p-5 border border-gray-100 mb-6">
                    <div>
                        <label className={labelCls}>Operations</label>
                        <input
                            type="text"
                            value={formData.purpose || ""}
                            onChange={(e) => setFormData(p => ({ ...p, purpose: e.target.value }))}
                            placeholder="General operations details..."
                            className={inputCls}
                        />
                    </div>
                </div>
                ...
                {/* Stored DC Entries Registry */}
                <div className="mt-10">
                    <div className="flex items-center gap-3 mb-4">
                        <h2 className="text-base font-black text-gray-800">
                            {formData.customer_name
                                ? `Standby DC Entries — ${formData.customer_name}`
                                : "All Standby DC Entries"}
                        </h2>
                        ...
                        <table className="w-full border-collapse">
                            <thead className="bg-gray-50 sticky top-0">
                                <tr>
                                    ...
                                    <th className="p-3 text-[11px] font-black text-gray-500 uppercase text-left border-b border-r">Order No</th>
                                    <th className="p-3 text-[11px] font-black text-gray-500 uppercase text-left border-b border-r">Order Type</th>
                                    <th className="p-3 text-[11px] font-black text-gray-500 uppercase text-left border-b border-r">Payment Terms</th>
                                    <th className="p-3 text-[11px] font-black text-gray-500 uppercase text-left border-b">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                {allDcData.length === 0 ? (
                                    <tr><td colSpan={7} className="p-8 text-center text-gray-400 text-sm">No Standby DC entries found.</td></tr>
                                ) : (
                                    allDcData.map((dc, i) => (
                                        <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                                            <td className="p-3 text-[12px] font-semibold border-r">{dc.standby_dc_no}</td>
                                            ...
                                            <td className="p-3 text-[12px] border-r">{dc.order_no}</td>
                                            <td className="p-3 text-[12px] border-r font-medium text-blue-600">{dc.order_type || "Service"}</td>
                                            <td className="p-3 text-[12px] border-r text-gray-650">{dc.payment_terms || "—"}</td>
                                            <td className="p-3">
                                                <div className="flex gap-2">
                                                <button
                                                    onClick={() => requirePassword(() => loadDcForEdit(dc.standby_dc_no))}
                                                ...
                                                >
                                                    ...
                                                    <button
                                                        onClick={() => { setViewDcNo(dc.standby_dc_no); setDcModalMinimized(false); setShowDcFormat(true); }}
...
export default StandbyDcEntryForm;
